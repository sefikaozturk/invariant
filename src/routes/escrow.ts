...
import { users, listings, transactions, balanceLedger } from '../db/schema.js';
...
escrowRouter.get('/onboard/status', requireAuth, async (c) => {
  ...
  if (!dbUser.stripeAccountId) return c.json({ onboarded: false, hasAccount: false });
...
escrowRouter.post('/create', requireAuth, async (c) => {
  ...
  if (existing) {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'usd', unit_amount: existing.amountCents,
          product_data: { name: listing.title, description: `Escrow payment — ${listing.provider}` },
        },
        quantity: 1,
      }],
      metadata: { transactionId: existing.id, listingId, buyerId: user.sub, sellerId: listing.userId },
      success_url: `${origin}/?escrow_paid=1`,
      cancel_url: `${origin}/?escrow_cancelled=1`,
    });
    return c.json({ transactionId: existing.id, checkoutUrl: session.url, amount: existing.amountCents });
  }
  ...
  const [tx] = await db.insert(transactions).values({
    listingId, buyerId: user.sub, sellerId: listing.userId,
    amountCents, platformFeeCents, sellerPayoutCents, status: 'pending_payment',
  }).returning();
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { transactionId: tx.id, listingId, buyerId: user.sub, sellerId: listing.userId },
    success_url: `${origin}/?escrow_paid=1`,
    cancel_url: `${origin}/?escrow_cancelled=1`,
  });
  ...
  return c.json({ transactionId: tx.id, checkoutUrl: session.url, amount: amountCents, fee: platformFeeCents, sellerPayout: sellerPayoutCents });
});
...
escrowRouter.post('/webhook', async (c) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // handle balance deposits
    if (session.metadata?.type === 'balance_deposit') {
      const userId = session.metadata.userId;
      const amount = Number(session.metadata.amountCents);
      if (userId && amount) {
        await db.update(users)
          .set({ balanceCents: sql`${users.balanceCents} + ${amount}` })
    ...
    if (txId) {
      ...
      if (tx && tx.status === 'pending_payment') {
        await db.update(transactions)
          .set({ status: 'paid', stripePaymentIntentId: session.payment_intent as string || null, updatedAt: new Date() })
...
escrowRouter.post('/:txId/confirm', requireAuth, async (c) => {
  ...
  if (tx.status !== 'delivered') return c.json({ error: 'Must be in delivered status' }, 400);
...
escrowRouter.post('/:txId/dispute', requireAuth, async (c) => {
  ...
  if (tx.status !== 'delivered' && tx.status !== 'paid') return c.json({ error: 'Cannot dispute' }, 400);
...
escrowRouter.post('/:txId/resolve', requireAuth, async (c) => {
  const user = c.get('user')!;
  const [dbUser] = await db.select({ username: users.username }).from(users).where(eq(users.id, user.sub)).limit(1);
  if (!dbUser || dbUser.username !== ADMIN_USERNAME) return c.json({ error: 'Admin only' }, 403);

  const txId = c.req.param('txId');
  const { action } = await c.req.json();
  const [tx] = await db.select().from(transactions).where(eq(transactions.id, txId)).limit(1);
  if (!tx) return c.json({ error: 'Transaction not found' }, 404);
  if (tx.status !== 'disputed') return c.json({ error: 'Only disputed txs can be resolved' }, 400);
...
escrowRouter.get('/my', requireAuth, async (c) => {
  ...
  const txs = await db.select({
    id: transactions.id, listingId: transactions.listingId,
    buyerId: transactions.buyerId, sellerId: transactions.sellerId,
    amountCents: transactions.amountCents, status: transactions.status,
    autoReleaseAt: transactions.autoReleaseAt,
    createdAt: transactions.createdAt, updatedAt: transactions.updatedAt,
    listingTitle: listings.title, listingProvider: listings.provider,
  })
    .from(transactions)
